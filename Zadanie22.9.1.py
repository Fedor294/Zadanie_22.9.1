while True:
    Posled = input("Введите несколько целых чисел через пробел,"
                   "если введете не целые числа или не цифры,"
                   "то программа вернется к началу ")
    Randomzifra = int(input("Введите любое случайное число в диапазоне чисел введенных ранее, "
                            "если введете слишком большое число, то программа вернется к началу "))
    Spisok = Posled.split()
    Spisok2 = ''.join(Spisok)
